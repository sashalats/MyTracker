...
final class CategorySelectionViewController: UIViewController {
    ...
    private let underLogoLabel: UILabel = {
        let label = UILabel()
        label.text = "Привычки можно объединить по смыслу"
...
extension CategorySelectionViewController: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.configure(title: category.title ?? "Без категории", isFirst: isFirst, isLast: isLast)
        return cell
    }
    ...
    private func confirmCategoryDeletion(_ category: TrackerCategoryCoreData) {
        let alert = UIAlertController(title: "Подтвердить удаление", message: nil, preferredStyle: .actionSheet)
