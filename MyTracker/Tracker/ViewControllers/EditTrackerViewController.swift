...
final class EditTrackerViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        if let tracker = editingTracker {
            ...
            // Подтягиваем выбранные значения
            selectedSchedule = tracker.schedule
            emojiAndColorPicker.selectedEmoji = tracker.emoji
            emojiAndColorPicker.selectedColor = tracker.color
            
            // Подтянем категорию из Core Data
            if let core = viewModel?.trackerStore.fetchTracker(withId: tracker.id) {
                if let coreCategory = core.category {
                    selectedCoreDataCategory = coreCategory
                    let rawTitle = coreCategory.title ?? ""
                    let trimmed = rawTitle.trimmingCharacters(in: .whitespacesAndNewlines)
                    let displayTitle = trimmed.isEmpty ? L10n.categoryTitleMissing : trimmed
                    selectedCategory = TrackerCategory(title: displayTitle, trackers: [])
                } else if let fallback = viewModel?.categoryStore.createCategoryIfNeeded(title: L10n.categoryTitleMissing) {
                    selectedCoreDataCategory = fallback
                    selectedCategory = TrackerCategory(title: fallback.title ?? L10n.categoryTitleMissing, trackers: [])
                }
                updateCategoryUI()
            }
        }
    ...
    @objc private func categoryTapped() {
        ...
        categoryVC.onCategorySelected = { [weak self] selectedCategory in
            self?.selectedCategory = TrackerCategory(
                title: selectedCategory.title ?? L10n.trackerNameMissing,
                trackers: []
            )
    ...
    @objc private func scheduleTapped() {
        ScheduleViewController.present(from: self,
                                       preselected: Set(selectedSchedule)) { [weak self] selected in
    ...
    private func updateScheduleUI() {
        let subtitle = selectedSchedule.isEmpty
        ? nil
        : selectedSchedule.map { $0.shortNameDay }.joined(separator: ", ")
        scheduleButtonView.updateSubtitle(subtitle)
    }
    ...
    private func updateCreateButtonState() {
        ...
        let enabled = nameFilled && categoryChosen && scheduleValid && emojiChosen && colorChosen
        saveButton.isEnabled = enabled
        saveButton.backgroundColor = enabled ? .blackDayNew : .grayText
        saveButton.setTitleColor(.whiteDayNew, for: .normal)
    }
